...
from src.models.model_3 import Model3


def weighted_mse_loss(y_hat, y, weights):
    return (weights * ((y_hat - y) ** 2).sum(1)).mean()
...
class Experiment4:
    ...
    def train(self, n_epochs=4, fine_tune_epochs=1, evaluate_every=5, gradient_clip=None):
        ...
        for epoch in range(n_epochs):
            ...
            epoch_loss = 0.0
            # Only fine tune the BERT backend in the ~~last~~ (First??) K epochs
            # fine_tune = (epoch + fine_tune_epochs) >= n_epochs
            fine_tune = (epoch < fine_tune_epochs)
            if fine_tune:
